    ...
    if ~contains(folders(folder).name, ".")
        ...
        if i >= 1 % for resuming after a failed batch
            if i == 10 % for specially treating one set of data
                ...
                close all; [force_data(i,1:8) gradients(i,1:8)] = process_data(folders(folder).name, 1, 1, [1:5], 2000000);
            end
        end
    end
end
...
%{
%plot method
fig = figure('units','centimeter','position',[0,0,12,8]);
set(fig,'PaperSize',[12 8]);
yplot=gradients(:,2);
yplot(yplot==0)=nan;
scatter(175+(1:length(gradients(:,2))).*25,yplot, 'filled')
hold on;
for i = 1:4
    yplot=gradients(i,4:8)/1000000;
    yplot(yplot==0)=nan;
    scatter(ones(1,5)*(175+25*i),yplot, '.', 'k')
end
xlabel("Height from Base (mm)")
ylabel("F/motor")
title("Force Available at Increasing Radius")
title("Gradient of Linear Region")
xlim([0 300])
ylim([0 0.004])
print(fig,'Gradient_of_Linear_Region_Axial.pdf','-dpdf') 

fig2 = figure('units','centimeter','position',[0,0,12,8]);
set(fig2,'PaperSize',[12 8]);
yplot=force_data(:,2);
yplot(yplot==0)=nan;
scatter(175+(1:length(force_data(:,2))).*25,yplot, 'filled')
hold on;
for i = 1:4
    yplot=force_data(i,4:8);
    yplot(yplot==0)=nan;
    scatter(ones(1,5)*(175+25*i),yplot, '.', 'k')
end
xlim([0 300])
